// app/production-planning/overseas/page.tsx
'use client';
...
import { overseasPlanApi, overseasItemApi } from '@/services/apiProductionPlanning';
import {
...
function OverseasProductionPlanContent() {
    ...
    const fetchPlans = useCallback(async (params?: { search?: string; status?: string }) => {
        try {
            setLoading(true);
            const data = await overseasPlanApi.getPlans(params);
            setPlans(data);
        } catch (error) {
    ...
    const fetchManufacturingItems = useCallback(async () => {
        try {
            const data = await overseasItemApi.getActiveItems();
    ...
    const handleDelete = useCallback(async () => {
        ...
        try {
            await overseasPlanApi.deletePlan(deleteConfirm.plan.id);
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                    <Box sx={{ width: '100%', mr: 1 }}>
                        <LinearProgress
                            ...
                            color={params.value && params.value >= 100 ? 'success' : 'secondary'}
                        />
                    </Box>
                    <Box sx={{ minWidth: 35 }}>
    ...
    return (
        <Box sx={{ width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
                <PublicIcon color="secondary" fontSize="large" />
            ...
            <Paper sx={{ p: 2, mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <Button variant="contained" color="secondary" startIcon={<SearchIcon />} onClick={handleSearch}>
                            検索
                        </Button>
                    ...
                    <Box>
                        ...
                        <Button variant="contained" color="secondary" startIcon={<AddIcon />} onClick={handleNewPlan}>
                            新規計画
                        </Button>
                    </Box>
                </Box>
...
export default function OverseasProductionPlanPage() {
    return (
        <AuthGuard>
            <Sidebar>
                <Suspense fallback={<Box sx={{ p: 2 }}>読み込み中...</Box>}>
                    <OverseasProductionPlanContent />
                </Suspense>
            </Sidebar>
        </AuthGuard>
    );
