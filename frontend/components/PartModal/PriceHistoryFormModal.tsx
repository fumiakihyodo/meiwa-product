// components/PriceHistoryFormModal.tsx
'use client';
...
import { Part, PriceHistory, PriceHistoryCreateData, PriceHistoryUpdateData } from '@/types/purchases';
...
export const PriceHistoryFormModal: React.FC<PriceHistoryFormModalProps> = ({
    open,
    onClose,
    onSuccess,
    part,
    ...
    }, [open, isEdit, priceHistory, part.id, reset]);
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            console.log('[PriceHistoryFormModal] File selected:', {
                name: file.name,
                size: file.size,
                type: file.type,
            });
    ...
    const handleRemoveFile = () => {
        console.log('[PriceHistoryFormModal] Removing file');
        setSelectedFile(null);
    ...
    const onSubmit = async (data: PriceFormData) => {
        try {
            console.log('[PriceHistoryFormModal] Submit started');
            console.log('[PriceHistoryFormModal] Form data:', data);
            console.log('[PriceHistoryFormModal] Selected file:', selectedFile);

            const submitData: PriceHistoryCreateData | PriceHistoryUpdateData = {
                part: data.part,
                price: Number(data.price), // 明示的にNumberに変換
            ...
            // ファイルが選択されている場合のみ追加
            if (selectedFile) {
                console.log('[PriceHistoryFormModal] Adding file to submit data:', {
            ...
            console.log('[PriceHistoryFormModal] Final submit data:', {
                ...submitData,
                quote_file: selectedFile ? `File: ${selectedFile.name}` : 'No file',
            });

            if (isEdit && priceHistory) {
                console.log('[PriceHistoryFormModal] Updating price history:', priceHistory.id);
                await purchasesApi.updatePriceHistory(priceHistory.id, submitData);
                toast.success('価格履歴を更新しました');
            } else {
                console.log('[PriceHistoryFormModal] Creating new price history');
                const result = await purchasesApi.createPriceHistory(submitData as PriceHistoryCreateData);
                console.log('[PriceHistoryFormModal] Create result:', result);
            ...
            console.error('[PriceHistoryFormModal] Submit error:', error);
            ...
            if (error && typeof error === 'object' && 'response' in error) {
                ...
                console.error('[PriceHistoryFormModal] Error response:', axiosError.response?.data);
