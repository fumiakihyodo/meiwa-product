...
export default function ManufacturingItemModal({
    ...
    // 製造拠点表示名を取得
    const getProductionTypeLabel = (value: ProductionType) => {
        const option = PRODUCTION_TYPE_OPTIONS.find(opt => opt.value === value);
        return option ? option.label : value;
    };

    // 海外サプライヤー表示名を取得
    const getOverseasSupplierName = (id: number | undefined) => {
        if (!id) return '-';
        const supplier = overseasSuppliers.find(s => s.id === id);
        return supplier ? `${supplier.supplier_name} - ${supplier.branch_name}` : '-';
    };

    // 製品表示名を取得
    const getProductName = (id: number | undefined) => {
        if (!id) return '-';
                ...
                <DialogContent sx={{ pt: 3 }}>
                    {isViewMode && item ? (
                        ...
                        <Box>
                            ...
                            {/* 製造拠点 */}
                            <SectionCard isEditMode={false} icon={<BuildIcon />} title="製造拠点">
                                <Grid container spacing={2}>
                                    <Grid item xs={12} sm={6}>
                                        <InfoRow
                                            isEditMode={false}
                                            label="製造拠点"
                                            value={getProductionTypeLabel(item.production_type)}
                                        />
                                    </Grid>
                                    {(item.production_type === 'overseas' || item.production_type === 'both') && (
                                        <>
                                            <Grid item xs={12} sm={6}>
                                                <InfoRow
                                                    isEditMode={false}
                                                    label="海外サプライヤー"
                                                    value={getOverseasSupplierName(item.overseas_supplier_branch)}
                                                />
                                            </Grid>
                                            <Grid item xs={12} sm={6}>
                                                <InfoRow
                                                    isEditMode={false}
                                                    label="仕入れ単価"
                                                    value={item.purchase_price !== undefined && item.purchase_price !== null ? `¥${Number(item.purchase_price).toLocaleString()}` : '-'}
                                                />
                                            </Grid>
                                        </>
                                    )}
                            ...
                            <SectionCard isEditMode={false} icon={<InventoryIcon />} title="在庫情報">
                                <Grid container spacing={2}>
                                    {(item.production_type === 'domestic' || item.production_type === 'both') && (
                                        <Grid item xs={12} sm={4}>
                                            <InfoRow
                                                isEditMode={false}
                                                label="国内在庫"
                                                value={`${item.domestic_stock || 0} ${item.unit}`}
                                            />
                                        </Grid>
                                    )}
                                    {(item.production_type === 'overseas' || item.production_type === 'both') && (
                                        <Grid item xs={12} sm={4}>
                                            <InfoRow
                                                isEditMode={false}
                                                label="海外在庫"
                                                value={`${item.overseas_stock || 0} ${item.unit}`}
                                            />
                                        </Grid>
                                    )}
                                    <Grid item xs={12} sm={4}>
                                        <InfoRow
                                            isEditMode={false}
                                            label="合計在庫"
                                            value={`${(item.domestic_stock || 0) + (item.overseas_stock || 0)} ${item.unit}`}
                                        />
                                    </Grid>
                                </Grid>
                            </SectionCard>
